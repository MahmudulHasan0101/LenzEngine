...
namespace lenz
{
	...
	public:
		...
		static void Draw(uint32_t indecisCount, const VertexArray& varr, const Shader& shader)
		{
			shader.Bind();
			varr.Bind();
		...
		static void Draw(uint32_t instanceCount, uint32_t indecisCount, const VertexArray& varr, const Shader& shader)
		{
			shader.Bind();
			varr.Bind();
